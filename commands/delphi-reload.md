...
Signal this Claude Code session's shim to recycle its DelphiLSP child.
...
If no shim is running for this Claude Code session yet, the command reports that — there's nothing to recycle.

After signaling, retry the LSP query that prompted this command. If the first retry returns empty results, give DelphiLSP another second or two and retry once more.
